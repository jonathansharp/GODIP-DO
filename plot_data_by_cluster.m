...
function plot_data_by_cluster(param_props,file_date,...
...
parfor p = 1:length(pressures)
    ...
    lon_temp = convert_lon(all_data.lon,'format','0-360');
    lon_temp(lon_temp < 20) = lon_temp(lon_temp < 20) + 360;
    % use depth for CMIP models
    title(['Data by Cluster at ' num2str(pressures(p)) ' m'],'fontsize',16);
    ...
    c.TickLength = 0;
    % save figure
    dname = [param_props.dir_name '/Figures/Clusters/' vrs '_c' ...
